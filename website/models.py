...
class TournamentResult(db.Model):
    ...
    Group_Pos = db.Column(db.String(1)) # 1 = Winner, 2 = Runner Up, + = Advanced, - = Eliminated, Space = Unkown 
    OnevOne = db.Column(db.String(1)) # Number = Number of Points, + = Advanced, - = Eliminated, Space = Unkown
    Top12 = db.Column(db.Integer) #0 - Unknown
...
class Transaction(db.Model):
    ...
    LoanDate = db.Column(db.DateTime(timezone=True), default=func.now())
    Marble_ID = db.Column(db.Integer, db.ForeignKey('marble.ID')) # Maybe lowercase ID?
...
class Draft(db.Model):
    ...
    Type = db.Column(db.String(20)) # Lottery, Draft Points, Marble Money, Draft Point Run-Off
